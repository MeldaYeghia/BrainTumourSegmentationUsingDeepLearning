"""
Evaluation script for the DeepLab-ResNet network on the extracted validation and testing sets
   of the BraTS 2015 dataset.

This script evaluates the model on 52800 test images.
...
pathOfSegsFolder = "../ResultingSegmentations/ResNet/TestSet"
# path of the folder in the Plots directory in which the results will be saved
scoresFolderName = 'ResNet'
...
savingOutput = 'SavedParametersFromTesting.txt'
...
def main():
    ...
    for step, lblName in zip(range(args.num_steps), lblNames):
        ...
        preds, gt, img = sess.run([pred, label_batch, image_batch])
        ...
        saveContouredSample(img, preds, gt, uniqueImageName, pathofContours, "DeepLab")
    ...
    endTime = time.time()
    print("Time taken to evaluate the current testing set: {} seconds".format(endTime - startTime))
    ...
    with open(savingDicesOfStepsPath, 'w') as f:
        for str1 in DicesOfSteps:
            f.write("%s\n" % str1)

    with open(savingNamesOfStepsPath, 'w') as f:
        for str2 in NamesOfSteps:
            f.write("%s\n" % str2)
